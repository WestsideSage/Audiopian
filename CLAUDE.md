...
# commit-helpers, vad-helpers, vad-health, anchor-selection, lyric-paint-helpers, realtime-whisper,
# telemetry-replay, lyric-annotations, mishearing)
```
...
- **`youtube-source.js`** (`YouTubeIframeSource`, `ytStateToString`, `isEmbedDisabledError`, `ensureYouTubeApi`) — the YouTube IFrame implementation of that contract. Clock = `getCurrentTime()` (frame-grained; **no** `performance.now()` interpolation — see ADR-0002). Tested in `tests/test_youtube_source.cjs`.
...
- **`scoring.js`** (`window.KaraokeeScoring`) — phonetic/fuzzy matching + line scoring engine: the stable four-character `doubleMetaphone` fast path plus untruncated `doubleMetaphoneFull`, `editDistance`, `wordsMatch`/`wordsMatchScore`, word normalization, syllable-weighted `interpolateWordTimings`, `computeLineScore`, `findMatchInWindow`, `mergeConfirmedMatches`. **Single source of truth** for these — `player.js` binds them. Tested in `test_scoring.cjs`.
- **`lattice-align.js`** (`window.KaraokeeLatticeAlign`) — pure Smith-Waterman phoneme-lattice fallback for ASR word-boundary drift. It applies a whole-line Dice guard plus per-anchor support, returns the aligned ASR token spans, and keeps a tighter four-second rescue horizon when consumed by the phrase engine. Tested in `test_lattice_align.cjs` plus phrase-engine cheese gates.
- **`phrase-engine.js`** (`window.KaraokeePhraseEngine`) — phrase-level scoring built on `scoring.js`: anchor matching, line settle/commit, late-evidence reconciliation, and the guarded lattice fallback. Tested in `test_phrase_engine.cjs` / `test_phrase_score.cjs`.
...
- **`metadata-clean.js`** (`window.KaraokeeMetadataClean`) — pure `cleanMetadata(rawTitle, rawAuthor)` / `stripNoise`: multi-separator artist/title split (ASCII/en/em-dash, pipe, colon) that **prefers an in-title "Artist - Title" over the channel** (fixes the channel-as-artist bug on non-ASCII-dash titles) + a **targeted** noise denylist (strips "(Official Audio)", "ft. X", remaster tags, etc. but never blanket-strips parens, so "(I Can't Get No) Satisfaction" survives). `youtube-meta.js parseTitleArtist` delegates to it; used on the paste-fallback path. Tested in `test_metadata_clean.cjs`.
- **`lyric-annotations.js`** (`window.KaraokeeLyricAnnotations`) — pure `isSpeakerLabel` / `isSectionHeader` / `isNonLyricLine` / `stripNonLyricLines`: drops **non-lyric LRC lines** so the scorer never treats them as required-to-sing. Catches rap-battle/dialogue **speaker labels** (a whole line ending in `:`, ≤4 words, no sentence stopword — so `Lil'D:` / `Shawty e demoni:` go but `and then she said:` / `Hold on:` stay) and whole-line **section headers** (balanced `[Chorus]`/`(Verse 1)`/`{Hook}` whose inner word is a section keyword — sung parentheticals like `(Soul) Ah ah ah ah!` survive). Fail-safe: an all-annotation sheet is returned unfiltered. Called from `lyrics-client.js parseLrc` (via a lazy resolver) and mirrored in `lyrics.py parse_lrc` for the dev path. Tested in `test_lyric_annotations.cjs` (+ Python parity in `test_lyrics.py`).
- **In-app search (lyrics-first):** `index.html` searches lrclib via `lyrics-client.js searchSongs(query)` (ranked synced-lyric candidates — every result is guaranteed singable), then on pick calls the **`vocalz-resolve` Cloudflare Worker** (`workers/resolve/`, `GET /api/resolve`) which holds the YouTube Data API key server-side, finds embeddable videos, and ranks them best-first (`rank.js`: duration-match + official/Topic channel + junk-title penalty) into a thumbnail picker. Paste-a-URL is now a **fallback revealed only on a search/resolve miss**; a curated "Popular picks" starter row seeds first-timers. The Worker is a **standalone deploy** (`npx wrangler deploy` from `workers/resolve/`; key via `wrangler secret put YOUTUBE_API_KEY`), independent of the static site. Tested: `test_lyrics_search.cjs`, `test_resolve_rank.cjs`, `test_resolve_worker.cjs`. Plan: `docs/superpowers/plans/2026-06-23-in-app-search.md`.
- **`vad-health-helpers.js`** (`window.KaraokeeVadHealth`) — pure `vadHealthVerdict({gameActive, vadEverFired, asrActive, secsSinceFirstAsrText})` → `ok | pending | sensor-dead` behind the in-game **mic-sensor warning**: flags a run where the recognizer is producing text but the energy sensor (neural VAD / RMS fallback) never fired — a dead game-mic stream (per-origin permission/device/AudioContext), NOT a quiet singer (silence alone never flags). Drives the `asr-provider-display` chip via `player.js _updateVadHealth` (100ms poll); the scoring engine independently fails open on the same condition (`phrase-engine.js` `_vadFlowSeen`). Tested in `test_vad_health.cjs`.
- **`mic-check-helpers.js`** (`window.KaraokeeMicCheck`) — pure `micCheckVerdict({recognizerAvailable, peakLevel, transcript, elapsedMs})` → `no-recognizer | listening | silent | capturing | recognized` for the **pre-game Mic Check** (a "Check your mic" panel in the player's prep overlay). `player.js _runMicCheck` opens its **own** short-lived mic stream + analyser (level meter) + Web Speech recognizer (transcript) for the check, then tears them down on Done — standalone, because the game's capture stack is `start()`-gated and is **not** running during the prep overlay. No scoring. Tested in `test_mic_check_helpers.cjs`.
