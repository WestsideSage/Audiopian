...
- **`mic-check-helpers.js`** (`window.KaraokeeMicCheck`) — pure `micCheckVerdict({recognizerAvailable, peakLevel, transcript, elapsedMs})` → `no-recognizer | listening | silent | capturing | recognized` for the **pre-game Mic Check** (a "Check your mic" panel in the player's prep overlay). `player.js _runMicCheck` reuses the already-open VAD analyser for a live level meter and taps the active recognizer's transcript (Web Speech `onresult` + realtime-delta hooks) — observe-only, no new mic session, no scoring. Tested in `test_mic_check_helpers.cjs`.
