...
# Karaokee v2 â€” Design Document
**Date:** 2026-02-19

## Overview

Three improvements to the Karaokee app:
1. **Bug fixes** â€” smart title/artist parsing from YouTube titles + stale audio fix
2. **Vocal removal** â€” demucs `htdemucs` model with toggle UI, processes in background
3. **YouTube search** â€” search by song name, pick from results list, auto-load (stretch feature)
...
1. If `info` has a populated `artist` field â†’ use it (existing behaviour, unchanged)
2. Else if `title` contains ` - ` â†’ split on first ` - `: left = artist, right = title
3. Else â†’ uploader as artist, full title as title (existing fallback)
...
**Dependency:** `demucs` (pip install). Uses `htdemucs` model (~300MB, downloaded once to `~/.cache/torch/hub`). Processing time ~60â€“90s on CPU.
...
- New button in controls bar: **"ðŸŽ¤ Remove Vocals"**
- On click:
  1. Button changes to "â³ Processing..." (disabled)
  2. `POST /separate` called
  3. Poll `GET /separate-status` every 2s
  4. On `"done"`: save `audio.currentTime`, swap `audio.src` to `/instrumental?t=<ts>`, `audio.load()`, seek back to saved time, resume playback, button changes to "ðŸŽµ Full Mix"
  5. On `"error"`: button resets, show error message
- Clicking "ðŸŽµ Full Mix" swaps back to `/audio?t=<original_ts>`, resumes at current position
...
```
[Search for a song...          ] [ðŸ” Search]

â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
â”‚ Black Moon - Who Got Da Props           â”‚
â”‚ BlackMoonVEVO Â· 4:05                    â”‚
â”œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤
â”‚ Who Got Da Props - Black Moon (Live)    â”‚
â”‚ OldSchoolHipHop Â· 3:58                  â”‚
â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜

â”€â”€ or paste a URL directly â”€â”€
...
3. Immediately calls `loadSong()` â€” no extra click
...
| `vocal_remover.py` | New file â€” demucs wrapper |
...
> **Note on demucs first run:** The first time `separate()` is called, demucs downloads the `htdemucs` model (~300MB) to `~/.cache/torch/hub`. This happens automatically â€” no manual step needed.
...
**Problem:** Browser caches `/audio` â€” navigating back then loading a new song plays the old audio under new lyrics.
...
### Task 5: Player UI â€” Remove Vocals Toggle
...
function toggleVocals() {
    if (usingInstrumental) {
        ...
        vocalBtn.textContent = 'ðŸŽ¤ Remove Vocals';
        return;
    }

    if (instrumentalReady) {
        // Already processed â€” just switch
        switchToInstrumental();
        return;
    }

    // Start processing
    vocalBtn.textContent = 'â³ Processing...';
    ...
    fetch('/separate-status')
        ...
        .then(data => {
            if (data.status === 'done') {
                ...
                vocalBtn.textContent = 'âŒ Failed';
                vocalBtn.disabled = false;
            } else {
...
function switchToInstrumental() {
    ...
    vocalBtn.textContent = 'ðŸŽµ Full Mix';
    vocalBtn.disabled = false;
}
```
...
3. Verify "ðŸŽ¤ Remove Vocals" button appears in the controls bar
4. Click it â€” verify button changes to "â³ Processing..."
5. Wait ~60â€“90s â€” verify button changes to "ðŸŽµ Full Mix" and audio switches
6. Click "ðŸŽµ Full Mix" â€” verify it switches back
...
<body>
    <div class="card">
        ...
        <div class="search-row">
            ...
            <button onclick="searchSongs()">ðŸ”</button>
        </div>
...
4. Click a result â€” verify URL, artist, title fields fill in and song starts loading
...
1. Search for a song â†’ results appear â†’ click result â†’ song loads correctly with proper artist/title
2. Paste a `"Artist - Title"` format YouTube URL â†’ artist/title auto-split correctly
3. Complete a song â†’ go back â†’ load a different song â†’ new audio plays (not old)
4. On player, click "ðŸŽ¤ Remove Vocals" â†’ processing starts â†’ switches to instrumental when done
5. Click "ðŸŽµ Full Mix" â†’ switches back to full audio at same position
