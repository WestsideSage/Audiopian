...
- **Risk — cross-match within one evidence:** a repeated anchor word in a single result could land on an adjacent line. Bounded by monotonic forward-only attribution + the look-back + the ≥0.75 content gate. This is *misattribution, not inflation*: one token credits exactly one phrase (`consumedTokenIds` + break), so the cleared-count — and therefore the honest % — is unchanged even if the line is off by one.
- **Risk — cross-match across two evidences (the real inflation path):** dedup is keyed by `evidence.id`, so the *same* word arriving in two **separate** results (different ids → different tokenIds) is not deduped and could credit an un-sung earlier line that happens to hold that word. Rare in practice (browser SR does not re-report finalized segments; realtime-Whisper chunks do not overlap), but it is exactly the inflation the honesty gate exists to catch. So this is **structurally bounded, not provably honest**. The gate stays the **human sing-test + the `*_reconciled` telemetry tag**: at sing-test, deliberately skip some lines and scan the run JSON for any `*_reconciled` credit landing on a skipped line.
